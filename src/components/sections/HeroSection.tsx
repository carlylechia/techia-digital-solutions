...
const HERO_IMAGE_PATH = "/images/hero/futuristic-cityscape-with-tech-dashboard.png";
  ...
  en: [
    {
      ...
      description: "Premium digital architecture built for intelligent workflows."
    },
    {
      title: "Automation-first",
      description: "Reduce manual work with sharper operations and clearer visibility."
    }
  ],
  fr: [
    {
      title: "Systèmes prêts pour l’IA",
      description: "Une architecture digitale premium pensée pour des workflows intelligents."
    },
    {
      title: "Automatisation d’abord",
      description: "Réduisez le travail manuel avec des opérations plus claires et plus fluides."
    }
  ]
} as const satisfies Record<Locale, ReadonlyArray<{ title: string; description: string }>>;

export function HeroSection({ locale = "en" }: { locale?: Locale }) {
  const reduceMotion = useReducedMotion();
  const motionState = reduceMotion ? { initial: false } : { initial: false, animate: "show" as const };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0" style={{ background: "rgba(4, 8, 18, 0.68)" }} />
      <div
        className="absolute inset-0"
        style={{
          background: "linear-gradient(to right, rgba(4, 8, 18, 0.95), rgba(8, 20, 36, 0.72), rgba(4, 8, 18, 0.12))"
        }}
      />
      <div
        className="absolute inset-0"
        style={{
          background: "linear-gradient(to top, #040812 0%, rgba(4, 8, 18, 0.42) 38%, transparent 70%)"
        }}
      />
      <div
        className="absolute inset-0"
      ...
      <div className="container relative flex items-center py-14 sm:py-20" style={{ minHeight: HERO_MIN_HEIGHT }}>
        <motion.div
          className="w-full max-w-[46rem] text-center lg:max-w-[45rem] lg:text-left"
          variants={reduceMotion ? undefined : contentVariants}
          {...motionState}
        >
          <motion.p variants={reduceMotion ? undefined : fadeUpVariants} className="font-script text-[clamp(2.4rem,4vw,3.8rem)] text-[#FDBA74]">
          ...
          >
            <span aria-hidden="true" className="size-2 rounded-full bg-[#22D3EE] shadow-[0_0_18px_rgba(34,211,238,0.7)]" />
            {dict.hero.eyebrow}
          </motion.div>
          ...
          >
            <motion.span variants={reduceMotion ? undefined : headingLineVariants} className="block md:whitespace-nowrap">
          ...
          <motion.div variants={reduceMotion ? undefined : fadeUpVariants} className="headline-underline mx-auto mt-6 lg:mx-0" aria-hidden="true" />
          ...
          >
            <Globe2 aria-hidden="true" className="size-4 shrink-0 text-[#2DD4BF]" />
            <p className="m-0 max-w-[42rem]">
              {heroSupportLine[locale]}
            </p>
          </motion.div>
          ...
          >
            {serviceBadges.map((badge) => (
              ...
              >
                <span aria-hidden="true" className="size-1.5 rounded-full bg-[#22D3EE]" />
                {badge}
              </li>
            ))}
          </motion.ul>
            ...
            {signals.map((signal, index) => (
                ...
                transition={
                  reduceMotion
                    ...
                    : {
                        ...
                        x: { duration: 0.55, delay: 0.34 + index * 0.12, ease: [0.16, 1, 0.3, 1] },
                        y: {
                          duration: 7 + index,
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="m-0 text-sm font-semibold tracking-[0.01em] text-white">{signal.title}</p>
                    <p className="m-0 mt-1 text-sm leading-6 text-[#CBD5E1]">{signal.description}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <motion.div
          ...
          transition={reduceMotion ? undefined : { duration: 0.48, delay: 0.62 }}
        >
          <Link
