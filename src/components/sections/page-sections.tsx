  ...
  return (
    <section className="container">
      <div className="gradient-border rounded-[2rem]">
        <div className="elevated-panel relative overflow-hidden px-5 py-8 sm:px-7 md:px-10 md:py-12">
          ...
          <div
            className={`relative z-10 grid gap-8 ${section.imageUrl ? "lg:grid-cols-[1.05fr_0.95fr] lg:items-center" : ""}`}
          >
            <div className="max-w-4xl">
              {section.heading ? (
                <h1 className="text-balance text-[clamp(2.4rem,8vw,4.5rem)] font-semibold leading-[1.02] text-foreground">
                  {section.heading}
                </h1>
              ) : null}
              {section.body ? (
              ...
              {(section.ctaLabel || section.cta2Label) && (
                <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                  {section.ctaLabel && section.ctaHref ? (
                    <Link
                      href={resolveHref(section.ctaHref, locale)}
                  ...
                  {section.cta2Label && section.cta2Href ? (
                    <Link
                      href={resolveHref(section.cta2Href, locale)}
                      className="btn-secondary w-full justify-center px-5 py-3 sm:w-auto"
                    >
                      {section.cta2Label}
                    </Link>
                  ) : null}
...
function StatsSection({
  ...
  section: Extract<PageSection, { type: "stats" }>;
}) {
  return (
    <section className="container">
        ...
        {section.items.map((item, idx) => (
          <article key={idx} className="gradient-border rounded-[1.5rem]">
            <div className="elevated-panel h-full p-6 text-center">
              <p className="text-3xl font-semibold text-accent-2 md:text-4xl">
                {item.value}
              </p>
...
function CardsSection({
  ...
  section: Extract<PageSection, { type: "cards" | "cards_images" }>;
}) {
  return (
    <section className="container">
      {section.heading ? (
        ...
        <p className="eyebrow mx-auto mb-8 justify-center">{section.eyebrow}</p>
      ) : null}
  ...
  return (
    <section className="container">
      <div className="mx-auto max-w-4xl gradient-border rounded-[1.75rem]">
        <div className="elevated-panel p-6 sm:p-8 md:p-10">
          {section.heading ? (
            <SectionHeading
          ...
          <div
            className={`prose prose-neutral dark:prose-invert max-w-none ${section.heading ? "mt-6" : ""}`}
          >
            {section.body
              .split("\n")
              .filter(Boolean)
  ...
  return (
    <section className="container">
      <div className="gradient-border rounded-[1.9rem]">
        <div className="elevated-panel relative overflow-hidden px-5 py-10 text-center sm:px-7 md:px-10 md:py-12">
          ...
          <div className="relative z-10">
            <h2 className="text-balance text-[clamp(2.3rem,8vw,3.8rem)] font-semibold text-foreground">
              {section.heading}
            </h2>
            {section.body ? (
            ...
            <Link
              href={resolveHref(section.ctaHref, locale)}
              className="btn-primary mt-8 w-full justify-center px-5 py-3.5 sm:w-auto"
            >
              {section.ctaLabel}
            </Link>
          </div>
