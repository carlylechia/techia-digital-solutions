...
type Option = { id: string; name: string; slug?: string; displayName?: string; userId?: string | null };
...
export function BlogPostEditor({
  ...
  async function saveDraft(event?: React.FormEvent<HTMLFormElement>) {
    ...
    if (!result.ok) {
      setSaveState("failed");
      setStatusMessage({ kind: "error", text: valueOf(result) });
      return;
    }
  ...
  function deleteDraft() {
    ...
    startTransition(async () => {
      ...
      else setStatusMessage({ kind: "error", text: result.error });
    });
  }
    ...
    <form ref={formRef} onSubmit={saveDraft} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      <div className="grid gap-6">
        <section className="premium-card grid gap-5 p-5 sm:p-7" aria-labelledby="editor-basics-title">
          ...
          <label className="form-label">Title<input className={inputClass} name="title" value={form.title} onChange={(event) => update("title", event.target.value)} disabled={!editable} required minLength={5} maxLength={180} /></label>
        ...
        <section className="premium-card grid gap-5 p-5 sm:p-7" aria-labelledby="editor-media-title">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <label className="form-label">Featured image URL<input className={inputClass} name="featuredImageUrl" value={form.featuredImageUrl} onChange={(event) => update("featuredImageUrl", event.target.value)} disabled={!editable} placeholder="https://res.cloudinary.com/..." /></label>
            <label className="form-label">Featured image alt text<input className={inputClass} name="featuredImageAlt" value={form.featuredImageAlt} onChange={(event) => update("featuredImageAlt", event.target.value)} disabled={!editable} maxLength={300} /></label>
