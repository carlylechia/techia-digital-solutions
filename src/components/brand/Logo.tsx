...
import { logoAssets, logoIconThemeAssets, type BrandAsset, type LogoVariant } from "./logo-assets";
...
function resolveThemeAsset(variant: LogoVariant, theme: Exclude<LogoTheme, "auto">): BrandAsset {
  // The logo pack includes transparent dark-surface alternates for the horizontal lockup and icon mark only.
  if (variant === "horizontal") {
    return theme === "dark" ? logoAssets.monochromeWhite : logoAssets.horizontal;
  }
...
function getSizeClass(variant: LogoVariant, size: LogoSize) {
  ...
  if (variant === "dark" || variant === "light") return backgroundLockupSizeClasses[size];
...
export function Logo({
  ...
  const resolvedHref = decorative ? null : href === undefined ? (locale ? getLocalizedHref(locale) : "/") : href;
  ...
  const content = (
    <span className={cn(imageClassName, "logo-media relative z-[1]")}>
      <LogoImage asset={asset} alt={altText} priority={priority} decorative={decorative} />
    </span>
  );
  ...
  if (!resolvedHref) {
    return (
      <span
        className={cn("logo-lockup inline-flex shrink-0 items-center", interactive && "logo-lockup--interactive", className)}
