  ...
  return (
    <section className={cn("container py-16", className)}>
      <div className="gradient-border rounded-[2rem]">
        <div className="elevated-panel relative overflow-hidden p-8 md:p-10">
          ...
          <div className="relative z-[1]">
            ...
            <h2 className="mt-4 max-w-4xl text-balance text-4xl font-semibold md:text-6xl">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {items.map((item) => (
                <Link key={item.href} href={item.href} className="group gradient-border rounded-[1.6rem]">
                  <article className="surface-panel h-full p-6 transition duration-500 group-hover:-translate-y-1 group-hover:shadow-[0_28px_84px_rgba(8,20,36,0.12)]">
                    {item.eyebrow ? <p className="eyebrow mb-4">{item.eyebrow}</p> : null}
