...
export function PremiumPageCta({
  ...
  const primaryHref = primaryAction.href.startsWith("/") ? getLocalizedHref(locale, primaryAction.href) : primaryAction.href;
  const secondaryHref = secondaryAction?.href && secondaryAction.href.startsWith("/")
    ? getLocalizedHref(locale, secondaryAction.href)
    : secondaryAction?.href;
  const accentScript = locale === "fr" ? "rendons-le iconique" : "let's make it iconic";
      ...
      <div className="gradient-border rounded-[2.1rem]">
        <div className="elevated-panel relative overflow-hidden px-6 py-10 text-center md:px-10 md:py-14">
          ...
          <div className="relative z-10">
            ...
            {eyebrow ? <p className="eyebrow mx-auto mb-3 mt-3 justify-center">{eyebrow}</p> : null}
            ...
            <div className="headline-underline mx-auto mt-6" aria-hidden="true" />
            {description ? <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-muted md:text-lg">{description}</p> : null}

            <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
              <Link href={primaryHref} className="btn-primary justify-center px-5 py-3.5">
                {primaryAction.label}
              </Link>
              {secondaryAction && secondaryHref ? (
                <Link href={secondaryHref} className="btn-secondary justify-center px-5 py-3.5">
