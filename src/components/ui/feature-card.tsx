...
export function FeatureCard({ item, locale, href, hrefBase, cta }: { item: CardItem; locale: Locale; href?: string; hrefBase?: string; cta?: string }) {
  ...
  const card = (
    <article className="group premium-card relative flex h-full flex-col justify-between rounded-[1.7rem] p-6 transition duration-500 hover:-translate-y-2 hover:border-cyan-300/40 hover:shadow-[0_30px_90px_rgba(8,20,36,0.12)]">
      ...
      {resolvedHref ? (
        <span className="relative z-[1] mt-6 inline-flex items-center gap-2 text-sm font-semibold text-accent">
