...
import { getBlogAuthors, getBlogCategories, getBlogTags, getTranslationCandidates } from "@/lib/blog/admin-data";
...
export default async function NewAdminBlogPostPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [categories, tags, authors, translationCandidates] = await Promise.all([getBlogCategories(locale), getBlogTags(locale), getBlogAuthors(), getTranslationCandidates(locale)]);
  const defaultAuthor = authors.find((author) => author.isActive) || authors[0];
  const defaultPost = { id: "new", title: "", slug: "", excerpt: "", content: "<p></p>", locale, status: "DRAFT" as BlogPostStatusValue, version: 1, categoryId: null, authorId: defaultAuthor?.id || null, translationGroupId: "", translationSourcePostId: "", tagIds: [], relatedPostIds: [], seoTitle: "", seoDescription: "", focusKeyword: "", canonicalUrl: "", featuredImageUrl: "", featuredImagePublicId: "", featuredImageAlt: "", ogImageUrl: "", ogImagePublicId: "", ctaTitle: "", ctaDescription: "", ctaHref: "", ctaLabel: "", featured: false, allowIndex: true, nofollow: false, scheduledAt: "", publishedAt: "", authorName: defaultAuthor?.displayName || user.name, reviewNotes: [], revisions: [] };
  return <BlogWorkspaceShell locale={locale} user={user} mode="admin" title="Create an article" description="Create a human, useful article and route it to the right author, category, and commercial journey."><BlogPostEditor locale={locale} post={defaultPost} categories={categories.map((item) => ({ id: item.id, name: item.name, isActive: item.isActive }))} tags={tags.map((item) => ({ id: item.id, name: item.name }))} authors={authors.map((item) => ({ id: item.id, name: item.displayName, displayName: item.displayName, isActive: item.isActive }))} canManage={canManage} canPublish={canPublish} isNew previewBase={`/${locale}/admin/blog/preview`} translationCandidates={translationCandidates.map((candidate) => ({ id: candidate.id, title: candidate.title, slug: candidate.slug, locale: candidate.locale as BlogLocale, publishedAt: candidate.publishedAt?.toISOString() || null, authorName: candidate.author?.displayName || "Editorial team" }))} /></BlogWorkspaceShell>;
}
