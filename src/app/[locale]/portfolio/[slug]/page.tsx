...
export function generateStaticParams() { return locales.flatMap((locale) => getDictionary(locale).caseStudies.map((item) => ({ locale, slug: item.slug }))); }
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return createMetadata({
    locale,
    title: item ? `${item.title} ${dict.pages.portfolio.caseStudySuffix} — teChia` : `${dict.pages.portfolio.caseStudySuffix} — teChia`,
      ...
      <section className="container py-6">
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-5">
          {dict.pages.portfolio.detailSections.map((section) => (
            <article key={section} className="premium-card p-5"><h2 className="font-semibold text-primary">{section}</h2><p className="mt-3 text-sm leading-6 text-muted">{dict.pages.portfolio.detailNarrative}</p></article>
          ))}
        </div>
      </section>
      <PremiumPageCta
