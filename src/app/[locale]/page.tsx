...
import { BadgeCheck, Cloud, Quote, Search, ShieldCheck, Sparkles, Star } from "lucide-react";
...
import { loadHomepageFeedback, loadHomepageProjectCards, loadPortfolioCards } from "@/lib/public-showcase";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return createMetadata({ locale, title: dict.meta.title, description: dict.meta.description });
}
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const dict = getDictionary(locale);
  const [homepageProjects, portfolioProjects, homepageFeedback] = await Promise.all([
    loadHomepageProjectCards(locale),
    loadPortfolioCards(locale),
    loadHomepageFeedback(locale)
  ]);
  const marqueePrimary = [...dict.hero.trust, ...dict.stack.slice(0, 4), ...dict.method.slice(0, 3)];
  ...
  const credibilitySuites = locale === "fr"
    ? [
        {
          icon: Cloud,
          title: "Cloud & déploiement",
          description: "Une couche de livraison fiable pour les sites, portails, tableaux de bord et actifs médias.",
          items: ["Vercel", "Cloudflare", "AWS S3", "PostgreSQL"]
        },
        {
          icon: Sparkles,
          title: "IA & automatisation",
          description: "Des assistants, des couches intelligentes et des workflows orientés métier plutôt qu’un simple effet de mode.",
          items: ["OpenAI API", "pgvector", "Auth.js", "Resend"]
        },
        {
          icon: Search,
          title: "SEO & visibilité",
          description: "Une structure pensée pour l’indexation, la mesure et la crédibilité sur les marchés exigeants.",
          items: ["GA4", "Google Search Console", "Schema.org", "Core Web Vitals"]
        }
      ]
    : [
        {
          ...
          description: "A reliable delivery layer for websites, portals, dashboards, and media-heavy digital assets.",
          items: ["Vercel", "Cloudflare", "AWS S3", "PostgreSQL"]
        },
        {
          icon: Sparkles,
          title: "AI and automation",
          description: "Assistants, smart layers, and business-facing workflows instead of AI theatre for its own sake.",
          items: ["OpenAI API", "pgvector", "Auth.js", "Resend"]
        },
        {
          icon: Search,
          title: "SEO and discoverability",
          description: "An indexability, measurement, and trust layer shaped for search-conscious clients and markets.",
          items: ["GA4", "Google Search Console", "Schema.org", "Core Web Vitals"]
        }
      ];
  const socialProofCopy = locale === "fr"
    ? {
        testimonialsTitle: "Des témoignages clients qui renforcent la crédibilité avant même le premier appel.",
        testimonialsDescription: "Une sélection administrée de retours mis en avant pour la page d’accueil, avec la liberté de les activer ensuite pour le portfolio ou la page fondateur.",
        feedbackEyebrow: "Canal de retour",
        feedbackTitle: "Partager un retour dans un flux structuré et modéré.",
        feedbackDescription: "Le formulaire de retour suit la même logique que le reste du site : clair, sérieux, multilingue et prêt pour une revue avant publication.",
    ...
    : {
        testimonialsTitle: "Client testimonials that strengthen trust before the first call even happens.",
        testimonialsDescription: "A curated, admin-managed testimonial library powers this homepage and can be featured across the portfolio and founder experience when needed.",
        feedbackEyebrow: "Feedback channel",
        feedbackTitle: "Share feedback through a structured, moderated flow.",
        feedbackDescription: "The feedback form follows the same standard as the rest of the site: clear, serious, multilingual, and ready for review before anything is published.",
  ...
  const testimonialDotLabels = testimonialItems.map((_, index) =>
    locale === "fr" ? `Voir le témoignage ${index + 1}` : `View testimonial ${index + 1}`
  );
  const projectOptions = portfolioProjects.slice(0, 8).map((item) => ({ slug: item.slug, name: item.title }));
  const signatureSuites = locale === "fr"
    ? [
        {
          title: "Présence signature",
          description: "Des expériences premium qui transforment la première impression en confiance durable.",
          items: dict.services.slice(0, 3).map((item) => item.title)
        },
        {
          title: "Systèmes intelligents",
          description: "Une architecture pensée pour les tableaux de bord, l’automatisation et la visibilité métier.",
          items: dict.stack.slice(0, 4)
        },
        {
          title: "Confiance globale",
          description: "Des signaux de sérieux visibles sur tous les écrans, pour les marchés locaux et internationaux.",
    ...
    : [
        {
          ...
          description: "Premium experiences that turn first impressions into durable trust.",
          items: dict.services.slice(0, 3).map((item) => item.title)
        },
        {
          title: "System intelligence",
          description: "Architecture designed for dashboards, automation, and operational visibility.",
          items: dict.stack.slice(0, 4)
        },
        {
          title: "Global trust signals",
          description: "Confidence cues built for local markets and international buyers alike.",
          items: dict.securityPromise.slice(0, 3)
        }
      ];
  const pageDirectoryItems = [
    ...
    },
    {
      ...
      href: getLocalizedHref(locale, "/solutions"),
      eyebrow: locale === "fr" ? "Modules" : "Modules",
      tags: dict.solutions.slice(0, 3).map((item) => item.title)
    },
    {
      title: dict.nav.industries,
      description: dict.pages.industries.description,
      href: getLocalizedHref(locale, "/industries"),
      eyebrow: locale === "fr" ? "Secteurs" : "Industries",
    ...
    },
    {
      ...
      eyebrow: locale === "fr" ? "Preuves" : "Proof",
      tags: dict.pages.portfolio.detailSections.slice(0, 3)
    },
    {
    ...
    },
    {
      ...
      tags: [dict.pages.contact.emailLabel, dict.pages.contact.whatsappLabel, dict.pages.contact.callLabel]
    },
    {
    ...
    },
    {
      ...
      tags: [dict.sections.services, dict.sections.solutions, dict.sections.pricing]
    },
    {
      title: locale === "fr" ? "Portail client" : "Client portal",
      description:
        locale === "fr"
      ...
      tags: locale === "fr" ? ["Projets", "Fichiers", "Factures"] : ["Projects", "Files", "Invoices"]
    },
    {
      title: locale === "fr" ? "Agent IA" : "AI consultant",
      description:
        locale === "fr"
      ...
      tags: locale === "fr" ? ["Chat", "Brief", "Recommandation"] : ["Chat", "Brief", "Recommendation"]
    },
    {
  ...
  return (
    <main id="main-content">
          ...
          <div className="surface-panel relative overflow-hidden p-5 md:p-6">
            ...
            <div className="relative z-10">
              <p className="font-script text-3xl text-accent-3 md:text-4xl">
                {locale === "fr" ? "un mouvement sans fin" : "an infinite rhythm of capability"}
              </p>
              <p className="eyebrow mt-3">{dict.pages.home.servicesTitle}</p>
              <div className="mt-5 space-y-4">
                <InfiniteMarquee items={marqueePrimary} />
                <InfiniteMarquee items={marqueeSecondary} direction="right" speed="36s" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          <div className="gradient-border rounded-[1.75rem]">
            <div className="elevated-panel h-full p-7 md:p-8">
              ...
              <h2 className="mt-4 text-balance text-3xl font-semibold text-foreground md:text-5xl">{dict.home.problemTitle}</h2>
              <p className="mt-5 text-lg leading-8 text-muted">{dict.home.problemBody}</p>
            </div>
          </div>
          <div className="gradient-border rounded-[1.75rem]">
            <div className="elevated-panel h-full p-7 md:p-8">
              <p className="eyebrow">{dict.pages.home.servicesTitle}</p>
              <h2 className="mt-4 text-balance text-3xl font-semibold text-foreground md:text-5xl">{dict.home.digitalHomeTitle}</h2>
              <p className="mt-5 text-lg leading-8 text-muted">{dict.home.digitalHomeBody}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container py-16">
        <SectionHeading title={dict.home.digitalHomeTitle} description={dict.home.digitalHomeBody} />
        <div className="grid gap-5 md:grid-cols-3">
          {dict.solutions.slice(0, 6).map((item) => <FeatureCard key={item.slug} item={item} locale={locale} hrefBase="solutions" cta={dict.common.learnMore} />)}
        </div>
      </section>

      <section className="container py-16">
        <SectionHeading eyebrow={dict.sections.services} title={dict.pages.home.servicesTitle} description={dict.pages.home.servicesDescription} />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {dict.services.slice(0, 8).map((item) => <FeatureCard key={item.slug} item={item} locale={locale} hrefBase="services" cta={dict.common.learnMore} />)}
        </div>
      </section>
      ...
      <section className="container py-16">
        <SectionHeading eyebrow={dict.sections.demoLab} title={dict.pages.home.demoTitle} description={dict.pages.home.demoDescription} />
        <DemoGrid locale={locale} />
      </section>

      <section className="container py-16">
        <SectionHeading eyebrow={dict.sections.caseStudies} title={dict.pages.home.caseTitle} description={dict.pages.home.caseDescription} />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {homepageProjects.map((item) => <FeatureCard key={item.slug} item={item} locale={locale} hrefBase="portfolio" cta={dict.common.viewCaseStudy} />)}
        </div>
      </section>

      <section className="container py-16">
        <SectionHeading eyebrow={dict.sections.industries} title={dict.home.europeTitle} description={dict.home.europeBody} />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {dict.industries.map((item) => <FeatureCard key={item.slug} item={item} locale={locale} hrefBase="industries" cta={dict.common.learnMore} />)}
        </div>
      </section>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="gradient-border rounded-[2rem]">
            <div className="elevated-panel h-full p-8 md:p-10">
              <p className="font-script text-3xl text-accent-3 md:text-4xl">
                {locale === "fr" ? "des systèmes avec du panache" : "systems with theatrical polish"}
              </p>
              <p className="eyebrow mt-3">{dict.sections.method}</p>
              <h2 className="mt-4 text-balance text-4xl font-semibold md:text-6xl">
                <span className="headline-gradient">{dict.pages.home.methodTitle}</span>
              </h2>
              <div className="headline-underline mt-6" aria-hidden="true" />
              <p className="mt-6 text-base leading-8 text-muted md:text-lg">{dict.pages.home.servicesDescription}</p>
      ...
      <PageDirectorySection
        accentScript={locale === "fr" ? "parcourir chaque destination" : "explore every destination"}
      ...
      <section className="container py-16">
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="premium-card p-6 lg:col-span-1">
            ...
            <h2 className="text-3xl font-semibold text-primary">{dict.pages.home.methodTitle}</h2>
          </div>
          <div className="grid gap-4 lg:col-span-2">
            {dict.method.map((step, index) => (
              <div key={step} className="premium-card flex gap-4 p-5">
                <span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-accent/10 text-sm font-bold text-accent">{index + 1}</span>
      ...
      <section className="container py-16">
        <div className="premium-card grid gap-8 p-8 lg:grid-cols-2">
          <div>
            ...
            <h2 className="text-3xl font-semibold text-primary md:text-4xl">{dict.pages.home.stackTitle}</h2>
            ...
            <div className="mt-6 flex flex-wrap gap-2">{dict.stack.map((item) => <span key={item} className="trust-pill">{item}</span>)}</div>
          </div>
          <div>
            <p className="eyebrow mb-3"><ShieldCheck className="size-4" /> {dict.sections.security}</p>
            <h2 className="text-3xl font-semibold text-primary md:text-4xl">{dict.pages.home.securityTitle}</h2>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-muted md:text-base">
              {locale === "fr"
            ...
            <div className="mt-6 grid gap-2">{dict.securityPromise.map((item) => <div key={item} className="rounded-2xl border border-border bg-background p-3 text-sm text-muted">{item}</div>)}</div>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          {credibilitySuites.map((suite) => {
            ...
            return (
              <article key={suite.title} className="gradient-border rounded-[1.65rem]">
                <div className="surface-panel h-full p-6">
                  <div className="icon-chip rounded-2xl p-3">
                    <Icon className="size-5" />
                  </div>
                  <h3 className="mt-5 text-2xl font-semibold text-primary">{suite.title}</h3>
                  <p className="mt-3 text-sm leading-7 text-muted">{suite.description}</p>
                  <div className="mt-5 flex flex-wrap gap-2">
                    {suite.items.map((item) => (
                      <span key={item} className="trust-pill">
      ...
      <section className="container py-16">
        <SectionHeading eyebrow={dict.sections.pricing} title={dict.pages.home.pricingTitle} description={dict.pages.home.pricingDescription} />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-5">
          {dict.pricing.map((item) => <FeatureCard key={item.slug} item={item} locale={locale} href="/start-project" cta={item.cta || dict.common.requestQuote} />)}
        </div>
      </section>

      <section className="container py-16">
        <SectionHeading eyebrow={dict.sections.faq} title={dict.pages.home.faqTitle} />
        <div className="mx-auto grid max-w-4xl gap-4">
          {dict.faqs.map((faq) => (
            <details key={faq.question} className="premium-card p-5">
              <summary className="cursor-pointer font-semibold text-primary">{faq.question}</summary>
        ...
        <section className="container py-16">
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <SwipeDotsCarousel
              ...
              ariaLabel={locale === "fr" ? "Carrousel de témoignages clients" : "Client testimonials carousel"}
              autoplayMs={5600}
              pauseOnHover
              dotLabels={testimonialDotLabels}
            >
              {testimonialItems.map((item) => (
                <article key={item.id} className="gradient-border rounded-[1.75rem]">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="founder-recommendation-badge">
                        ...
                        {item.source.startsWith("seed_homepage")
                          ? locale === "fr" ? "Témoignage mis en avant" : "Featured testimonial"
                          : locale === "fr" ? "Retour approuvé" : "Approved feedback"}
                      </div>
                    ...
                    {item.rating ? (
                      <div className="mt-5 flex items-center gap-1 text-amber-400">
                        {Array.from({ length: item.rating }).map((_, starIndex) => (
                          <Star key={`${item.id}-star-${starIndex}`} className="size-4 fill-current" />
                        ))}
                      </div>
                    ) : null}

                    <p className="mt-5 text-base leading-8 text-primary md:text-lg">&ldquo;{item.quote}&rdquo;</p>
                    <div className="mt-6 border-t border-border pt-4">
                      <p className="text-sm font-semibold text-primary">{item.name}</p>
                      {item.roleLine ? <p className="mt-1 text-sm text-muted">{item.roleLine}</p> : null}
                    </div>
                  </div>
                </article>
              ))}
        ...
        <div className="gradient-border rounded-[2rem]">
          <div className="elevated-panel relative overflow-hidden p-6 md:p-10">
            ...
            <div className="relative z-10 grid gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
              <div>
                <p className="font-script text-3xl text-accent-3 md:text-4xl">{socialProofCopy.feedbackScript}</p>
                <p className="eyebrow mt-3">{socialProofCopy.feedbackEyebrow}</p>
                <h2 className="mt-4 max-w-3xl text-balance text-4xl font-semibold md:text-6xl">
                  <span className="headline-gradient">{socialProofCopy.feedbackTitle}</span>
                </h2>
                <div className="headline-underline mt-6" aria-hidden="true" />
                <p className="mt-6 max-w-2xl text-base leading-8 text-muted md:text-lg">{socialProofCopy.feedbackDescription}</p>

                <div className="mt-6 grid gap-3">
                  {socialProofCopy.feedbackHighlights.map((item) => (
                    <div key={item} className="subtle-tile rounded-[1.35rem] p-4 text-sm leading-7 text-muted">
                      {item}
                    </div>
                  ))}
                </div>
                ...
                <div className="surface-panel h-full p-4 sm:p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3 px-2 pb-3 pt-1">
                    ...
                    <span className="inline-flex items-center gap-2 text-sm font-medium text-muted">
                      ...
                      {locale === "fr" ? "Sérieux, propre, prêt pour publication" : "Clean, serious, and publication-ready"}
                    </span>
                  </div>

                  <CustomerFeedbackForm locale={locale} projectOptions={projectOptions} />
                </div>
              </div>
            </div>
          </div>
